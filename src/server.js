...
// 立即初始化 Notion skill 配置（在配置检查之前）
  const notionConfigPath = path.join(STATE_DIR, "openclaw.json");
  try {
    const notionConfig = {
      skills: {
        entries: {
    ...
    fs.writeFileSync(notionConfigPath, JSON.stringify(notionConfig, null, 2), { encoding: "utf8" });
    log.info("wrapper", `✓ Notion skill pre-configured`);
  } catch (err) {
    log.error("wrapper", `✗ Notion config error: ${err.message}`);
  }
  if (isConfigured()) {
    (async () => {
      try {
